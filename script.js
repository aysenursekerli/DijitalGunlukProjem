...
const AppManager = {
    ...
    openTemplateModal() {
        ...
        const specificTemplates = [
            {
                id: 'kalori', label: 'Kalori Takibi', emoji: '🍎',
                gradient: 'linear-gradient(135deg,#FFF5E6,#FFE4CC)',
                accent: '#FF6B35', sub: 'Günlük Beslenme Kaydı',
                pattern: 'template-kalori'
            },
            {
                id: 'spor', label: 'Spor Günlüğü', emoji: '🏃',
                gradient: 'linear-gradient(135deg,#EBF5FF,#CCEEFF)',
                accent: '#1565C0', sub: 'Aktivite & Egzersiz Takibi',
                pattern: 'template-spor'
            },
            {
                id: 'butce', label: 'Bütçe Takibi', emoji: '💰',
                gradient: 'linear-gradient(135deg,#F0FFF4,#D4EDDA)',
                accent: '#2E7D32', sub: 'Aylık Gelir & Gider Planı',
                pattern: 'template-butce'
            },
            {
                id: 'duygu', label: 'Duygu Günlüğü', emoji: '🌙',
                gradient: 'linear-gradient(135deg,#FDF0FF,#EDD9FF)',
                accent: '#7B1FA2', sub: 'Günlük Ruh Hali Takibi',
                pattern: 'template-duygu'
            },
            {
                id: 'okuma', label: 'Okuma Listesi', emoji: '📚',
                gradient: 'linear-gradient(135deg,#FFFBEB,#FEF3C7)',
                accent: '#E65100', sub: 'Kitap Takip Defteri',
                pattern: 'template-okuma'
            }
        ];
        ...
        specificTemplates.forEach(t => {
            ...
            item.className = 'template-item';
            item.innerHTML = `
                <div style="width:100%; height:240px; background:${t.gradient}; display:flex; flex-direction:column; align-items:center; justify-content:center; position:relative; overflow:hidden; gap:10px;">
                    <div style="position:absolute; top:0; left:0; right:0; height:4px; background:${t.accent}; opacity:0.6; border-radius:0;"></div>
                    <div style="background:white; width:54px; height:54px; border-radius:16px; display:flex; align-items:center; justify-content:center; font-size:1.9rem; box-shadow:0 4px 14px rgba(0,0,0,0.12);">
                        ${t.emoji}
                    </div>
                    <div style="text-align:center;">
                        <div style="font-weight:800; color:${t.accent}; font-size:0.95rem; margin-bottom:3px;">${t.label}</div>
                        <div style="font-size:0.7rem; color:#888; font-weight:500;">${t.sub}</div>
                    </div>
                    <div style="position:absolute; bottom:12px; background:rgba(255,255,255,0.7); padding:3px 12px; border-radius:20px; font-size:0.65rem; font-weight:700; color:${t.accent}; border:1px solid ${t.accent}30;">Şablonu Seç →</div>
                </div>
            `;
            item.addEventListener('click', () => {
                modal.classList.remove('active');
                this.addNewPageToBook('', t.pattern);
            });
            grid.appendChild(item);
        });
